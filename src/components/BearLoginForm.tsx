// src/components/BearLoginForm.tsx
"use client";
...
export default function BearLoginForm() {
  ...
  useEffect(() => {
    ...
    // 1) Grab DOM elements exactly like in the original JS
    ...
    const mouth = root.querySelector<SVGPathElement>(".mouth");
    const mouthMask = root.querySelector<SVGPathElement>("#mouthMaskPath");
    ...
    // === Original JS logic, lightly adapted to vanilla + requestAnimationFrame ===
    let caret = 0;

    const updateEyes = () => {
      const val = email.value;
      const len = val.length || 1;
      const ratio = Math.min(1, Math.max(0, caret / len)); // 0..1
      const x = -8 + ratio * 16; // -8..8

      eyeL.setAttribute("transform", `translate(${x},0)`);
      eyeR.setAttribute("transform", `translate(${x},0)`);
    };

    const onEmailInput = () => {
      caret = email.selectionStart ?? email.value.length;
      updateEyes();

      // mouth shapes (simplified – same breakpoints as demo)
      const v = email.value;
      if (!v) {
        mouth.setAttribute("d", "M195 200 Q200 205 205 200");
      } else if (v.includes("@")) {
        mouth.setAttribute("d", "M180 195 Q200 220 220 195");
      } else if (v.length > 4) {
        mouth.setAttribute("d", "M185 195 Q200 210 215 195");
      } else {
        mouth.setAttribute("d", "M190 195 Q200 205 210 195");
      }
    };
    ...
    const coverEyes = () => {
      armL.setAttribute("transform", "translate(-93,2) rotate(0)");
      armR.setAttribute("transform", "translate(93,2) rotate(0)");
    };

    const uncoverEyes = () => {
      armL.setAttribute("transform", "translate(-93,220) rotate(-105)");
      armR.setAttribute("transform", "translate(93,220) rotate(105)");
    };

    email.addEventListener("input", onEmailInput);
    email.addEventListener("click", onEmailClick);
    email.addEventListener("keyup", onEmailInput);
    email.addEventListener("focus", onEmailInput);
    ...
    return () => {
      ...
      email.removeEventListener("focus", onEmailInput);
  ...
  return (
    <div
      ref={containerRef}
      className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#cde8ff] via-[#e8f4ff] to-[#cde8ff] px-4">
      <div className="w-full max-w-md bg-white rounded-[32px] shadow-2xl p-8 md:p-10 space-y-8">
        {/* === 1) Paste the EXACT SVG from the article inside this div === */}
        <div className="mx-auto w-64 h-64 rounded-full border-[6px] border-[#3a5e77] bg-[#a9ddf3] overflow-hidden flex items-center justify-center">
        ...
        {/* === 2) Form – same structure as article, but with Astro-friendly names === */}
        <form method="post" action="/api/auth/login" className="space-y-6">
          <div className="space-y-2">
            <label
              htmlFor="email"
              className="block text-[#217093] font-bold text-lg">
              Email
            </label>
            <input
              id="email"
              name="user"
              type="text"
              className="w-full h-14 rounded-2xl bg-[#f3fafd] border-2 border-[#217093] px-4 text-lg font-semibold text-[#27496d] shadow-md focus:outline-none focus:border-[#4eb8dd] focus:ring-4 focus:ring-[#4eb8dd33]"
          ...
          <button
            type="submit"
            className="w-full h-14 rounded-2xl bg-[#4eb8dd] hover:bg-[#217093] text-white font-bold text-lg shadow-lg transition-colors">
            Log in
          </button>
          ...
          <p className="pt-2 text-center text-sm text-gray-600">
            ...
            <a
              href="/signup"
              className="text-[#4eb8dd] font-semibold hover:text-[#217093]">
              Sign up
            </a>
          </p>
        </form>
